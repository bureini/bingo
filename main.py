...
class _BingoGamePageState extends State<BingoGamePage> {
  ...
  String _gameStatusMessage = "Waiting for players to join...";

  // REPLACE: Modify with your production server URL when deployed live
  // Format matching our backend: ws://<host>/ws/<room_id>/<username>
  final String _wsUrl = 'ws://localhost:8080/ws/room101/Player_${100 + (new DateTime.now().millisecondsSinceEpoch % 900)}';
            ...
            switch (event) {
              case 'card_assigned':
                setState(() {
                  ...
                  // Auto-daub center free space securely matching server assignment
                  _daubedStates[2][2] = true;
                  _gameStatusMessage = "Card assigned! Connected to room: ${data['room_id']}";
                });
                break;
              ...
              case 'game_started':
                setState(() {
                  _gameStarted = true;
                  _gameStatusMessage = "Game is Live! Focus up!";
                });
                break;
  ...
  void _claimBingo() {
    ...
    // Inform the server we assert a victory condition
    final payload = {
      'action': 'claim_bingo'
    };
          ...
          children: [
            Container(
              ...
              child: Text(
                ...
                style: const TextStyle(color: Colors.whiteEpsilon, fontWeight: FontWeight.w500),
              ),
            ),
            Card(
